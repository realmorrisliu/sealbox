...
- [x] 2.1 `secret set <key>` reads the value from stdin only; remove the positional argument
- [x] 2.2 Fail with a message naming stdin when nothing is piped and the input is not a terminal
- [x] 2.3 Add `secret gen <key> --type password|hex [--length N] [--ttl N]`
- [x] 2.4 Fix `secret list` to call `GET /v1/secrets` and print what it returns, instead of claiming the server cannot list
- [x] 2.5 Confirm no command accepts a secret's value as an argument
...
- [x] 3.5 Generating over an existing key creates a new version, leaving the previous retrievable
- [x] 3.6 Listing returns metadata with no value, ciphertext, or encrypted data key in any entry
- [x] 3.7 An expired secret does not appear in a listing
- [x] 3.8 The generated password alphabet excludes the ambiguous characters

## 4. Documentation

- [x] 4.1 Update `docs/cli-reference.md` for `secret set` reading stdin and the new `secret gen`
- [x] 4.2 Update `docs/getting-started.md` where it shows storing a value
- [x] 4.3 Update `CLAUDE.md`: MVP item 3 done
- [x] 4.4 Final verification: `cargo fmt --check`, `cargo test --workspace`, `cargo clippy --all-targets --all-features --workspace -- -D warnings`
