...
The grant also names the **owner** — the role migrations run as. `GRANT … ON ALL TABLES` covers the
tables that exist at that moment and nothing created afterwards, so a role provisioned into an
empty database would end up seeing nothing at all; `ALTER DEFAULT PRIVILEGES FOR ROLE <owner>` is
what covers the rest. It cannot be guessed from the admin connection: default privileges attach to
whoever *creates* an object, and naming the wrong role fails silently.
